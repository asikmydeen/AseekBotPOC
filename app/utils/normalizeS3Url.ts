 ...
 *  - It normalizes the bucket name to the correct one (in our case, replacing 'aseekbot-files.s3' with 'aseekbot-files-ammydeen4.s3').
 *
 * Usage:
...
export function normalizeS3Url(url: string): string {
  ...
  // Replace 'aseekbot-files.s3' with 'aseekbot-files-ammydeen4.s3'
  if (urlWithoutParams.includes('aseekbot-files.s3')) {
    return urlWithoutParams.replace('aseekbot-files.s3', 'aseekbot-files-ammydeen4.s3');
  }
